...
import { AlertTriangle, CheckCircle2, Clock3, Filter, Search, User, CalendarDays, ListTodo, CircleAlert, Plus, Trash2, Lock } from "lucide-react";
...
function inferSection(taskName) {
...
function inferPriority(taskName) {
  const name = taskName.toLowerCase();
  if (name.includes("bank reconciliation") || name.includes("sales reconciliation") || name.includes("cash reconciliation") || name.includes("inventory") || name.includes("payroll") || name.includes("tm1") || name.includes("fcc") || name.includes("tax") || name.includes("working capital")) return 1;
...
const initialTasks = rawTasks.map(([task, user, approver], index) => ({
  id: index + 1,
...
function getNextMonthLabel(currentMonth) {
...
function resetTasksForNewMonth(sourceTasks) {
...
function createMonthArchive(monthLabel, sourceTasks) {
...
export default function MonthEndCloseCommandCenter() {
  ...
  const [monthHistory, setMonthHistory] = useState(storedState?.monthHistory || []);
  const [monthSnapshots, setMonthSnapshots] = useState(storedState?.monthSnapshots || { [startingMonth]: cloneTasks(initialTasks) });
  ...
  const employees = useMemo(() => ["All", ...Array.from(new Set(["Cindy", ...tasks.map((t) => t.user)]))], [tasks]);
  const sections = useMemo(() => ["All", ...Array.from(new Set(tasks.map((t) => t.section)))], [tasks]);
  const editableSections = useMemo(
    () => Array.from(new Set(["General", "Cash", "Leases", "Intercompany", "Payroll", "Inventory", "Prepaids", "Fixed Assets", "Tax", "Reporting", "Accruals", "AP", "Revenue", ...tasks.map((t) => t.section)])),
    [tasks]
  );
  ...
  const sectionBreakdown = useMemo(() => {
    const grouped = Array.from(new Set(tasks.map((t) => t.section))).map((name) => {
  ...
  const availableMonths = useMemo(() => {
    return [selectedMonth, ...monthHistory.map((entry) => entry.month)].filter(
      (month, index, arr) => arr.indexOf(month) === index
    );
  ...
  const updateFilteredTasks = (checked) => {
  ...
  const toggleSelectAllShown = (taskList, checked) => {
        ...
        >
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              ...
              <Button className="rounded-2xl" onClick={handleGenerateNextMonth}>Generate next month</Button>
            </div>
          </div>
        </motion.div>
                    ...
                    <div className="flex items-center gap-2 rounded-2xl bg-slate-50 px-3 py-2">
                      ...
                      {isMonthClosed ? (
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <Button className="rounded-xl" onClick={markMonthClosed}>Close month</Button>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {statusPieData.map((item, i) => (
                      <div key={item.name} className="flex items-center justify-between rounded-2xl bg-slate-50 px-3 py-2 text-sm">
                        <span className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: pieColors[i] }} />{item.name}</span>
                    ...
                    <div key={person.name} className="rounded-2xl bg-slate-50 p-4">
                      <div className="mb-2 flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-slate-500">{person.complete} of {person.total} complete</p>
                        </div>
                        <Badge variant="secondary" className="rounded-xl">{person.progress}%</Badge>
                      </div>
                ...
                <div className="space-y-4">
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <Button
                        variant="outline"
                        className="rounded-2xl"
                        onClick={() => setSortOwnerTasks((prev) => !prev)}
                      >
                      ...
                      <Button
                        variant="outline"
                        className="rounded-2xl"
                        onClick={() => setSortPriorityTasks((prev) => !prev)}
                      >
                  ...
                  <div className="grid gap-3 rounded-3xl bg-slate-50 p-4 lg:grid-cols-[1.8fr_1fr_1fr_1.2fr_auto]">
                    ...
                    <Select value={newTaskForm.user} onValueChange={(value) => setNewTaskForm((prev) => ({ ...prev, user: value }))}>
                      <SelectTrigger className="rounded-2xl bg-white"><SelectValue placeholder="Owner" /></SelectTrigger>
                      <SelectContent>
                        {employees.filter((e) => e !== "All").map((e) => <SelectItem key={e} value={e}>{e}</SelectItem>)}
                      </SelectContent>
                    </Select>
                    <Select value={newTaskForm.approver || "None"} onValueChange={(value) => setNewTaskForm((prev) => ({ ...prev, approver: value === "None" ? "" : value }))}>
                      <SelectTrigger className="rounded-2xl bg-white"><SelectValue placeholder="Approver" /></SelectTrigger>
                      <SelectContent>
                  ...
                  {bulkDeleteArmed ? (
                    <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-rose-200 bg-rose-50 px-3 py-3">
                      ...
                      <Input
                        ...
                        className="h-9 w-28 rounded-xl bg-white"
                      />
                      <Button size="sm" className="rounded-xl" onClick={confirmBulkDelete}>Confirm bulk delete</Button>
                      <Button size="sm" variant="outline" className="rounded-xl" onClick={cancelBulkDelete}>Cancel</Button>
              ...
              <CardContent className="space-y-4">
                <div className="grid gap-3 lg:grid-cols-4">
                  ...
                  <Select value={employee} onValueChange={setEmployee}>
                    <SelectTrigger className="rounded-2xl"><SelectValue placeholder="Employee" /></SelectTrigger>
                    <SelectContent>
                  ...
                  <div className="flex gap-3">
                    <Select value={section} onValueChange={setSection}>
                      <SelectTrigger className="rounded-2xl"><SelectValue placeholder="Section" /></SelectTrigger>
                      <SelectContent>
                      ...
                      <TableRow>
                        <TableHead>
                          <Checkbox
                            ...
                            onCheckedChange={(checked) => toggleSelectAllShown(filteredTasks, !!checked)}
                          />
                        </TableHead>
                        ...
                        <TableRow key={task.id}>
                            ...
                            <div className="space-y-2">
                              <div className="flex items-start gap-3">
                                <Input
                                  ...
                                  className="h-9 min-w-[540px] rounded-xl"
                                />
                                <Button
                                  size="icon"
                                  variant="ghost"
                                  className="shrink-0 rounded-xl text-slate-500 hover:text-rose-600"
                              ...
                              {deleteTargetId === task.id ? (
                                <div className="flex flex-wrap items-center gap-2 rounded-2xl bg-rose-50 p-2">
                                  ...
                                  />
                                  <Button size="sm" className="rounded-xl" onClick={confirmDeleteTask}>Confirm delete</Button>
                                  <Button size="sm" variant="outline" className="rounded-xl" onClick={cancelDeleteTask}>Cancel</Button>
                            ...
                            <Select value={task.section} onValueChange={(value) => updateTask(task.id, { section: value })}>
                              ...
                              <SelectContent>
                                {editableSections.map((sectionOption) => (
                                  <SelectItem key={sectionOption} value={sectionOption}>{sectionOption}</SelectItem>
                                ))}
                              </SelectContent>
                            </Select>
                          </TableCell>
                          ...
                          <TableCell>
                            {task.approver === "Jefferson" ? (
                              task.approved ? (
                                <Badge className="rounded-xl bg-emerald-100 text-emerald-800 hover:bg-emerald-100">
                                  Approved by Manager
                                </Badge>
                              ) : task.done ? (
                                <Badge className="rounded-xl bg-amber-100 text-amber-800 hover:bg-amber-100">
          ...
          <TabsContent value="employee" className="space-y-6">
            <div className="grid gap-6 lg:grid-cols-[300px_1fr]">
              <Card className="rounded-3xl border-0 shadow-sm">
                ...
                <CardContent className="space-y-4">
                  <Select value={employee} onValueChange={setEmployee}>
                    <SelectTrigger className="rounded-2xl"><SelectValue placeholder="Choose employee" /></SelectTrigger>
                    <SelectContent>
                      {employees.filter((e) => e !== "All").map((e) => <SelectItem key={e} value={e}>{e}</SelectItem>)}
                    </SelectContent>
                  </Select>
                            ...
                            <div className="flex gap-3">
                              ...
                              <div>
                                <div className="flex flex-wrap items-center gap-2">
                                  ...
                                  <Badge variant="secondary" className="rounded-xl">P{task.priority}</Badge>
                                </div>
                                <p className="mt-2 text-sm text-slate-500">Section: {task.section} {task.approver ? `• Approver: ${task.approver}` : ""}</p>
                                <Textarea
                                  value={task.notes}
                                  onChange={(e) => updateTask(task.id, { notes: e.target.value })}
                                  placeholder="Add notes, blockers, or exceptions"
                  ...
                  <div className="max-w-md rounded-3xl bg-slate-50 p-5">
                    ...
                    <div className="flex gap-3">
                      <Input
                        ...
                        className="rounded-2xl bg-white"
                      />
                      <Button className="rounded-2xl" onClick={unlockApprovalTab}>Unlock</Button>
                    </div>
                  ...
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                      ...
                      <p className="mt-2 text-sm text-slate-500">
                        Completed items waiting for your approval. Approvals here update the main task board automatically.
                      </p>
                    </div>
                    <div className="flex gap-2">
                      <Badge className="rounded-xl bg-amber-100 text-amber-800 hover:bg-amber-100">
                      ...
                      <Button variant="outline" className="rounded-2xl" onClick={lockApprovalTab}>Lock tab</Button>
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
                        ...
                        <div key={task.id} className="rounded-3xl border border-slate-200 bg-white p-5">
                          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                            <div>
                              <div className="flex flex-wrap items-center gap-2">
                                <p className="font-medium text-slate-900">{task.task}</p>
                                <Badge variant="secondary" className="rounded-xl"><span className={ownerColor[task.user] || "font-medium"}>{task.user}</span></Badge>
                  ...
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <Button
                        ...
                        className="rounded-2xl"
                        onClick={openBulkDelete}
                        disabled={selectedTaskIds.filter((id) => reportingTasks.some((task) => task.id === id)).length === 0}
                      >
                      ...
                      <Button
                        variant="outline"
                        className="rounded-2xl"
                        onClick={() => setSortOwnerReporting((prev) => !prev)}
                      >
                      ...
                      <Button
                        variant="outline"
                        className="rounded-2xl"
                        onClick={() => setSortPriorityReporting((prev) => !prev)}
                      >
                  ...
                  <Table className="min-w-[1700px]">
                    <TableHeader className="bg-slate-50">
                      <TableRow>
                        <TableHead>
                          <Checkbox
                            checked={reportingTasks.length > 0 && reportingTasks.every((task) => selectedTaskIds.includes(task.id))}
                            onCheckedChange={(checked) => toggleSelectAllShown(reportingTasks, !!checked)}
                          />
                        </TableHead>
                        <TableHead>Done</TableHead>
                        <TableHead>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  onClick={() => {
                    setMonthHistory([]);
                    setMonthSnapshots((prev) => ({ [selectedMonth]: cloneTasks(tasks) }));
                  }}
              ...
              {monthHistory.length === 0 ? (
                ...
                monthHistory.map((entry) => (
                  <div key={entry.month} className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3">
                    ...
                    <Badge variant="secondary" className="rounded-xl">{entry.progress}%</Badge>
                  </div>
                ))
              )}
            </CardContent>
          ...
          <Card className="rounded-3xl border-0 shadow-sm">
            <CardContent className="flex items-start gap-3 p-5">
              <div className="rounded-2xl bg-rose-100 p-3 text-rose-700"><CircleAlert className="h-5 w-5" /></div>
              <div>
          ...
          <Card className="rounded-3xl border-0 shadow-sm">
            <CardContent className="flex items-start gap-3 p-5">
              <div className="rounded-2xl bg-amber-100 p-3 text-amber-700"><ListTodo className="h-5 w-5" /></div>
              <div>
          ...
          <Card className="rounded-3xl border-0 shadow-sm">
            <CardContent className="flex items-start gap-3 p-5">
              <div className="rounded-2xl bg-slate-100 p-3 text-slate-700"><Filter className="h-5 w-5" /></div>
              <div>
